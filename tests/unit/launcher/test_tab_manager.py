...
from econsim.gui.launcher.tabs.base_tab import AbstractTab
from econsim.gui.launcher.tabs.manager import TabManager
...
def test_tab_manager_callbacks():
    ...
    callbacks_called = {"activated": [], "deactivated": [], "added": [], "removed": []}

    manager.set_tab_activated_callback(lambda tab_id: callbacks_called["activated"].append(tab_id))
    manager.set_tab_deactivated_callback(
        lambda tab_id: callbacks_called["deactivated"].append(tab_id)
    )
