...
class TestTradeEconomicCoherence:
    ...
    def setup_method(self):
        ...
        for env_var in [
            "ECONSIM_TRADE_DRAFT",
            "ECONSIM_TRADE_EXEC",
            "ECONSIM_TRADE_GUI_INFO",
            "ECONSIM_FORAGE_ENABLED",
        ]:
    ...
    def test_trade_execution_changes_inventories(self):
        ...
        sim = SimulationCoordinator.from_config(
            config, agent_positions=agent_positions
        )
        ...
        for agent in sim.agents:
            initial_inventories[agent.id] = dict(agent.carrying_inventory)
        ...
        for step_num in range(20):  # Run enough steps for potential trades
            ...
            for agent in sim.agents:
                current_inventory = dict(agent.carrying_inventory)
        ...
        #         final_inventories = {
        #             agent.id: dict(agent.carrying_inventory) for agent in sim.agents
        #         }
        #
        #         # At minimum, ensure no agent has impossible negative inventory
        #         for agent_id, inventory in final_inventories.items():
        #             assert all(
        #                 count >= 0 for count in inventory.values()
        #             ), f"Agent {agent_id} has negative inventory after trades: {inventory}"
        #
        #         print(
        #             f"Economic coherence validated: {trades_executed} trades executed with real consequences"
        #         )
    ...
    def test_trade_consequences_affect_decision_making(self):
        ...
        # Run simulation for a few steps to allow initial trades
        for step_num in range(10):
            sim.step(rng)

        # Capture initial decision states
        initial_decision_states = {}
        for agent in sim.agents:
            initial_decision_states[agent.id] = {
                "mode": getattr(agent, "mode", "unknown"),
                "inventory": dict(agent.carrying_inventory),
                "home_inventory": dict(agent.home_inventory),
            }

        # Run additional steps and capture decision states
        agent_behaviors = []
        ...
        print(
            f"Behavioral coherence validated: {len(modes_observed)} modes, {len(positions_visited)} positions"
        )
    ...
    def test_no_phantom_trade_effects(self):
        ...
        #     final_metrics = {
        #         "intents_generated": getattr(
        #             sim.metrics_collector, "trade_intents_generated", 0
        #         ),
        #         "trades_executed": getattr(sim.metrics_collector, "trades_executed", 0),
    ...
    def test_deterministic_trade_economic_outcomes(self):
        ...
        for run_num in range(3):
            ...
            rng = random.Random(77777)  # Use same seed for each run
            ...
            for agent in sim.agents:
                run_data["final_inventories"][agent.id] = dict(agent.carrying_inventory)
    ...
    def test_trade_enables_vs_disabled_economic_differences(self):
        ...
        sim_no_trade = SimulationCoordinator.from_config(
            config, agent_positions=agent_positions
        )
        rng_no_trade = random.Random(55555)

        # Run simulation without trade execution
        for _ in range(12):
            sim_no_trade.step(rng_no_trade)

        # Run with trading enabled
        os.environ["ECONSIM_TRADE_EXEC"] = "1"  # Re-enable for comparison

        sim_with_trade = SimulationCoordinator.from_config(
            config, agent_positions=agent_positions
        )
        rng_with_trade = random.Random(55555)  # Same seed

        # Run simulation with trade execution
        for _ in range(12):
            sim_with_trade.step(rng_with_trade)

        # Compare economic outcomes
        no_trade_inventories = {
            agent.id: dict(agent.carrying_inventory) for agent in sim_no_trade.agents
        }
        with_trade_inventories = {
            agent.id: dict(agent.carrying_inventory) for agent in sim_with_trade.agents
        }
        ...
        if with_trade_executed > no_trade_executed:
            print(
                f"Economic impact validated: {with_trade_executed} trades executed vs {no_trade_executed}"
            )
        else:
