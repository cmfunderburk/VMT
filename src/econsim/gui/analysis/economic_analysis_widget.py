...
STUB: This widget depended on delta playback which has been removed.
TODO: Re-implement with DebugRecorder system.
"""
...
class EconomicAnalysisWidget(QWidget):
    """Stub: Economic analysis widget (delta recorder removed)."""
    ...
    def update_overview(self, step: int):
        ...
        for decision in economic_decisions[:5]:  # Show last 5 decisions
            decisions_text += (
                f"Agent {decision.get('agent_id', '?')}: {decision.get('decision_type', '?')}\n"
            )
    ...
    def update_trades(self, step: int):
        ...
        for row, trade in enumerate(trade_events):
            ...
            utility_delta = trade.get("seller_utility_after", 0) - trade.get(
                "seller_utility_before", 0
            )
        ...
        for intent in trade_intents:
            intents_text += (
                f"Agent {intent.get('agent_id', '?')} → Agent {intent.get('partner_id', '?')}\n"
            )
    ...
    def update_performance(self, step: int):
        ...
        if performance_metrics:
            ...
            metrics_text += (
                f"Step Duration: {performance_metrics.get('step_duration_ms', 0):.2f} ms\n"
            )
            metrics_text += f"Agents Processed: {performance_metrics.get('agents_processed', 0)}\n"
            metrics_text += (
                f"Resources Processed: {performance_metrics.get('resources_processed', 0)}\n"
            )
            ...
            metrics_text += (
                f"Memory Usage: {performance_metrics.get('memory_usage_mb', 0):.2f} MB\n"
            )
        else:
