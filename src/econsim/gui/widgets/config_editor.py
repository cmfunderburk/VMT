...
from econsim.gui.launcher.framework.phase_manager import PhaseManager

# Import from framework using proper package paths
from econsim.gui.launcher.framework.test_configs import ALL_TEST_CONFIGS, TestConfiguration
...
class ConfigurationValidator(QWidget):
    ...
    def setup_ui(self):
        ...
        self.status_label.setStyleSheet(
            "color: #81c784; background: #1a3d1a; padding: 8px; border-radius: 4px;"
        )
    ...
    def validate_config(self, config: TestConfiguration) -> dict[str, list[str]]:
        ...
        if custom_phases is not None:
            if len(custom_phases) == 0:
                issues["errors"].append(
                    "Custom phases configured but empty - add at least one phase"
                )
            else:
        ...
        if distance_k == 0.0:
            ...
            issues["info"].append(
                f"k={distance_k:.1f}: Strong local preference - agents heavily favor nearby targets"
            )
        elif distance_k >= 1.0:
            issues["info"].append(
                f"k={distance_k:.1f}: Moderate distance penalty - balanced targeting behavior"
            )
        else:
            issues["info"].append(
                f"k={distance_k:.1f}: Light distance penalty - slight preference for nearby targets"
            )
    ...
    def update_validation(self, config: TestConfiguration):
        ...
        if issues["errors"]:
            ...
            self.status_label.setStyleSheet(
                "color: #ff6b6b; background: #3d1a1a; padding: 8px; border-radius: 4px;"
            )
        elif issues["warnings"]:
            self.status_label.setText("⚠️ Configuration Has Warnings")
            self.status_label.setStyleSheet(
                "color: #ffb74d; background: #3d2f1a; padding: 8px; border-radius: 4px;"
            )
        else:
        ...
        if not details_html:
            details_html = (
                "<span style='color: #81c784;'>All parameters are within recommended ranges.</span>"
            )
...
class ConfigurationPreview(QWidget):
    ...
    def setup_ui(self):
        ...
        self.preview_text.setStyleSheet(
            "background: #1e1e1e; color: #ffffff; border: 1px solid #555555; border-radius: 4px;"
        )
...
class PresetManager(QWidget):
    ...
    def _get_presets_file(self) -> Path:
        ...
        # This file is in src/econsim/gui/widgets/
    ...
    def import_presets(self):
        ...
        try:
            ...
            QMessageBox.information(
                self, "Import Complete", f"Imported {imported_count} presets successfully."
            )

        except Exception as e:
            QMessageBox.warning(self, "Import Error", f"Failed to import presets: {e}")

    def export_presets(self):
        """Export presets to file."""
        if not self.presets:
            QMessageBox.information(self, "No Presets", "No presets to export.")
            return

        file_path, _ = QFileDialog.getSaveFileName(
            self, "Export Presets", "vmt_config_presets.json", "JSON Files (*.json);;All Files (*)"
        )
        ...
        try:
            ...
            QMessageBox.information(
                self, "Export Complete", f"Exported {len(self.presets)} presets to {file_path}"
            )

        except Exception as e:
            QMessageBox.warning(self, "Export Error", f"Failed to export presets: {e}")
...
class ConfigEditor(QWidget):
    ...
    def __init__(self, initial_config: TestConfiguration | None = None, parent=None):
        super().__init__(parent)
    ...
    def create_controls_panel(self) -> QWidget:
        ...
        self.grid_width_slider = ParameterSlider(
            "Grid Width", 5, 50, 20, 1, "", "Width of the simulation grid (5-50 cells)"
        )
        grid_layout.addWidget(self.grid_width_slider)

        self.grid_height_slider = ParameterSlider(
            "Grid Height", 5, 50, 20, 1, "", "Height of the simulation grid (5-50 cells)"
        )
        ...
        self.agent_count_slider = ParameterSlider(
            "Agent Count", 1, 100, 20, 1, "", "Number of agents in the simulation (1-100)"
        )
        agent_layout.addWidget(self.agent_count_slider)

        self.perception_slider = ParameterSlider(
            "Perception Radius", 1, 20, 8, 0.5, "", "How far agents can see (1-20 cells)"
        )
        ...
        self.preference_combo.addItems(["mixed", "cobb_douglas", "leontief", "perfect_substitutes"])
        ...
        self.phase_summary_label.setStyleSheet(
            "color: #cccccc; font-size: 9px; font-style: italic;"
        )
    ...
    def on_parameter_changed(self):
        ...
        self.current_config = TestConfiguration(
            ...
            grid_size=(
                int(self.grid_width_slider.get_value()),
    ...
    def configure_phases(self):
        ...
        try:
            ...
            QMessageBox.warning(
                self, "Phase Configuration", f"Phase configuration editor not available: {e}"
            )
        except Exception as e:
            QMessageBox.critical(self, "Error", f"Error opening phase configuration: {e}")
        ...
        try:
            if self.custom_phases:
                ...
                self.phase_summary_label.setText(
                    f"✅ {phase_count} custom phases ({total_turns} turns)"
                )
            else:
    ...
    def launch_custom_test(self):
        ...
        try:
            ...
            temp_file = (
                manual_tests_dir / f"temp_custom_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.py"
            )
            ...
            QMessageBox.critical(self, "Launch Error", f"Failed to launch custom test: {e}")

    def save_custom_test(self):
        """Save current configuration as a permanent custom test."""
        if not self.validator.update_validation(self.current_config):
            QMessageBox.warning(
                self, "Invalid Configuration", "Please fix the configuration errors before saving."
            )
            return
        ...
        try:
            ...
            QMessageBox.critical(self, "Save Error", f"Failed to save custom test: {e}")
    ...
    def generate_test_code(self, config: TestConfiguration) -> str:
        ...
        if custom_phases:
            phases_import = (
                "from econsim.gui.launcher.framework.phase_manager import PhaseDefinition\n"
            )
            phases_code = "# Custom phase configuration\nCUSTOM_PHASES = [\n"
            for phase in custom_phases:
                phases_code += "    PhaseDefinition(\n"
...
from econsim.gui.launcher.framework.base_test import StandardPhaseTest
...
'''
    ...
    def save_as_preset(self):
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            QMessageBox.information(
                self, "Preset Saved", f"Configuration saved as preset '{preset.name}'"
            )
