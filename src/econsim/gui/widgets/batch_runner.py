#!/usr/bin/env python3
"""Batch Test Runner Widget for VMT Framework.

Professional batch test execution system with progress tracking, estimated
completion time, pause/resume capabilities, and detailed reporting.
...
This module was extracted from MANUAL_TESTS/batch_test_runner.py for proper
...
class BatchTestExecutor(QObject):
    ...
    def _execute_single_test(self, test_name: str, config: TestConfiguration) -> BatchTestResult:
        ...
        result = BatchTestResult(
            test_name=test_name, config=config, start_time=datetime.now(), status="running"
        )

        try:
            # Construct test command - find test file in MANUAL_TESTS
            test_file = f"test_{config.id}_framework_version.py"
            # Get project root and look in MANUAL_TESTS
            project_root = Path(__file__).parent.parent.parent.parent.parent
            test_path = project_root / "MANUAL_TESTS" / test_file
...
class BatchProgressWidget(QWidget):
    ...
    def update_times(self):
        ...
        if self.current_session.completed_tests > 0:
            ...
            remaining_tests = (
                self.current_session.total_tests - self.current_session.completed_tests
            )
...
class BatchRunner(QWidget):
    """Main batch test runner interface.

    This is the public interface for the batch runner component.
    Renamed from BatchTestRunner for consistency with module naming.
    """
    ...
    def init_ui(self):
        ...
        self.resume_btn.setStyleSheet(
            "QPushButton { background-color: #17a2b8; color: white; font-weight: bold; padding: 8px; }"
        )
    ...
    def start_batch_execution(self):
        ...
        if not selected_tests:
            QMessageBox.warning(
                self, "No Tests Selected", "Please select at least one test to execute."
            )
            return
        ...
        self.current_session = BatchExecutionSession(
            session_id=session_id, start_time=datetime.now(), total_tests=len(selected_tests)
        )
        ...
        self.log_widget.add_log_entry(
            "SYSTEM", f"🚀 Batch execution started with {len(selected_tests)} tests"
        )
    ...
    def on_session_completed(self, session: BatchExecutionSession):
        ...
        self.log_widget.add_log_entry(
            "SYSTEM", f"🎉 Batch execution completed! Success rate: {success_rate:.1f}%"
        )
