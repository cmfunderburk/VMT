...
try:
    ...
    from PyQt6.QtWidgets import QGridLayout, QScrollArea, QTextEdit, QVBoxLayout, QWidget
    ...
    Qt = QTimer = object  # type: ignore

    # Create a mock pyqtSignal that can be called
    class MockSignal:
        def __init__(self, *args):
            pass

    pyqtSignal = MockSignal  # type: ignore
...
class TestGalleryWidget(QWidget):  # pragma: no cover - GUI component extracted from monolith
    ...
    def _calculate_optimal_layout(self) -> dict:
        ...
        try:
            ...
            min_columns = max(
                2, available_width // 450
            )  # Minimum 2 columns, more for very wide screens
            max_columns = min(
                6, available_width // 280
            )  # Maximum 6 columns, limited by minimum card width
                ...
                if used_width <= available_width:
                    ...
                    if waste_percent < 15:
                        ...
                        if waste_percent < best_solution["waste_percent"] or (
                            abs(waste_percent - best_solution["waste_percent"]) < 5
                            and cols > best_solution["columns"]
                        ):
                            best_solution = solution
    ...
    def populate_tests(self):
        ...
        for (
            config
        ) in ALL_TEST_CONFIGS.values():  # Builtin list still authoritative for card ordering
            try:
        ...
        try:
            ...
            try:
                ...
                self._resize_timer.start(
                    150
                )  # Reduced delay for more responsive updates  # type: ignore[attr-defined]
            except Exception:
        ...
        try:
            if self.test_cards:
                ...
                if hasattr(self, "_last_width"):
                    ...
                    if width_change > 50 or self._would_cards_be_cut_off():
