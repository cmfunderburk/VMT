...
class PhaseManager:
        ...
        for i, phase in enumerate(sorted_phases):
            if i == 0:
                ...
                if phase.turn_start != prev_phase.turn_end + 1:
                    raise ValueError(
                        f"Phase {phase.number} starts at {phase.turn_start}, expected {prev_phase.turn_end + 1}"
                    )
    ...
    def create_simple_phases(cls, **kwargs) -> "PhaseManager":
        ...
        active_phases = [
            (behavior, duration) for behavior, duration in phase_sequence if duration > 0
        ]
    ...
    def check_transition(self, current_turn: int, current_phase: int) -> PhaseTransition | None:
        """Check if we need to transition to a new phase."""
        for phase_num, phase_def in self.phases.items():
            if current_turn == phase_def.turn_start and current_phase == phase_num - 1:
        ...
        for phase in sorted_phases:
            behavior_name = (
                ...
                if phase.forage_enabled and phase.trade_enabled
                    ...
                    "Forage"
                    if phase.forage_enabled
                    else "Exchange" if phase.trade_enabled else "Idle"
                )
            )
    ...
    def get_current_phase_info(self, current_turn: int) -> PhaseDefinition | None:
    ...
    def get_current_features(self, turn: int) -> "SimulationFeatures":
        """Get current simulation features for a given turn.

        Returns SimulationFeatures object with behavior flags set according to
        the current phase. V2 system uses these features directly, not environment variables.

        Args:
            turn: Current simulation turn

        Returns:
            SimulationFeatures object with current phase behavior
        """
        from ....simulation.features import SimulationFeatures

        phase = self.get_current_phase_info(turn)
        if phase is None:
            # Default to unified behavior (both foraging and trading enabled)
            return SimulationFeatures(
                forage_enabled=True, trade_draft_enabled=True, trade_execution_enabled=True
            )

        # V2 system: Configure features based on phase
        return SimulationFeatures(
            forage_enabled=phase.forage_enabled,
            trade_draft_enabled=phase.trade_enabled,
            trade_execution_enabled=phase.trade_enabled,
        )
