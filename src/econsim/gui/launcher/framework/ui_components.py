...
class ControlPanel(QWidget):
    ...
    def setup_ui(self):
        ...
        self.speed_combo.setCurrentIndex(3)  # Default to 20 turns/second
        speed_layout.addWidget(self.speed_combo)
        layout.addLayout(speed_layout)

        # Current speed indicator
        self.speed_indicator = QLabel("⏱️  Speed: 20 turns/second")
        ...
        self.status_text = QLabel(
            f"Click 'Start Test' to begin the 900-turn test sequence (~{format_duration(initial_duration)})."
        )
    ...
    def update_display(
        ...
        from .test_utils import format_duration, get_estimated_duration, get_timer_interval
...
class PlaybackControlPanel(QWidget):
    ...
    def setup_ui(self):
        ...
        self.play_button.setStyleSheet(
            "QPushButton { padding: 8px 12px; background-color: #4CAF50; color: white; font-weight: bold; }"
        )
        ...
        self.speed_combo = QComboBox()
        self.speed_combo.addItems(
            ["🐌 2 steps/sec", "🚶 8 steps/sec", "🏃 20 steps/sec", "💨 Unlimited"]
        )
    ...
    def update_progress(
        ...
        phase_info: tuple[int, str] = None,
    ):
        """Update progress display."""
        progress = (current_step / total_steps * 100) if total_steps > 0 else 0

        # Include phase information if provided
        if phase_info:
            phase_num, phase_desc = phase_info
            self.progress_label.setText(
                f"Step: {current_step} / {total_steps} ({progress:.1f}%) - Phase {phase_num}"
            )
        else:
...
class TestLayout(QWidget):
    ...
    def setup_ui(self):
        ...
        self.pygame_placeholder.setStyleSheet(
            "border: 1px solid #555555; background: #2a2a2a; color: #ffffff;"
        )
