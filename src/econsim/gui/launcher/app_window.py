...
try:
    ...
    from .tabs import BatchRunnerTab, ConfigEditorTab, GalleryTab, TabManager
...
class VMTLauncherWindow(QMainWindow):  # pragma: no cover - GUI application window
    ...
    def __init__(self, parent=None):
        ...
        self.gallery_widget: TestGalleryWidget | None = None
        self.status_area: QTextEdit | None = None
    ...
    def _handle_gallery_status(self, message: str) -> None:
        ...
        if message == "COMPARISON_LIMIT":
            QMessageBox.information(
                self, "Comparison Limit", "Maximum 4 tests can be compared simultaneously."
            )
    ...
    def _launch_test_subprocess_fallback(self, config: Any, test_name: str) -> None:
        ...
        try:
            ...
            # Construct test file path (go up from src/econsim/gui/launcher/ to project root)
            test_path = (
                Path(__file__).parent.parent.parent.parent.parent / "MANUAL_TESTS" / test_file
            )
            ...
            if self.launcher_logger:
                self.launcher_logger.info(
                    f"✅ Subprocess launched successfully - PID: {process.pid}"
                )
        ...
        if _launcher_modules_available and self._comparison_controller:
            ...
            if add_result.added:
                ...
                if add_result.reason == "capacity":
                    QMessageBox.information(
                        self, "Comparison Full", "Maximum 4 tests can be compared simultaneously."
                    )
                    ...
                    QMessageBox.warning(
                        self, "Invalid Selection", f"Cannot add '{test_name}' to comparison."
                    )
        else:
            # Fallback logic
    ...
    def launch_comparison(self) -> None:
        ...
        if len(selection) < 2:
            QMessageBox.warning(
                self, "Insufficient Selection", "Please select at least 2 tests for comparison."
            )
            return
        ...
        try:
            ...
            if result.success:
                labels = [
                    f"{name}({vers})"
                    for name, vers in zip(
                        selection, ["original", "framework"] * len(selection), strict=False
                    )
                ]
                ...
                error_msg = (
                    "; ".join(result.errors) if result.errors else "Unknown comparison launch error"
                )
                self.log_status(f"✗ Failed to launch comparison: {error_msg}")
                self.statusBar().showMessage("Failed to launch comparison")
    ...
    def log_status(self, message: str) -> None:
        ...
        if self.status_area:
            ...
            if (
                hasattr(self.status_area, "document")
                and self.status_area.document().lineCount() > 20
            ):
